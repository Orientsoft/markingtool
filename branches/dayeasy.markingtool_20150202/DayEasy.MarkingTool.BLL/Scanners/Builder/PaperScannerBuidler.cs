...
namespace DayEasy.MarkingTool.BLL.Scanners.Builder
{
    ...
    {
        private static Logger _logger = Logger.L<PaperScannerBuidler>();
        public PaperScannerBuidler(string imagePath, PaperUsageInfo usage,string classId)
            : base(imagePath, usage,classId)
        {
        }
        ...
        public override JsonResultBase LoadImage()
        {
            ...
            var bmp = Image.FromFile(ImagePath);
            //自动纠偏
            ...
            var path = Path.Combine(DeyiKeys.ItemPath, ImageName);
            bmp.Save(path);
            bmp.Dispose();
            SetImagePath(path);
        ...
        {
            using (var paperImage = Image.FromFile(ImagePath))
            {
                var scanner = new LineScanner();
                var result = scanner.Scan(paperImage);
                if (!result.Status)
                {
        ...
        {
            var result = new SliceMap().SliceAction(ImagePath, lines.ToList());
            if (result.Status)
            {
        ...
        {
            var path = Path.Combine(DeyiKeys.SavePath, ImageName, DeyiKeys.RecognitionName, "0.jpg");
            if (!File.Exists(path))
                return new JsonResult<StudentInfo>("基础信息识别错误！");
            using (var basiceImage = Image.FromFile(path))
            {
        ...
        {
            var path = Path.Combine(DeyiKeys.SavePath, ImageName, DeyiKeys.RecognitionName, "1.jpg");
            if (!File.Exists(path))
                return new JsonResults<bool>("答题卡识别区域异常！");
            try
            {
                var answerSheetImage = Image.FromFile(path);
