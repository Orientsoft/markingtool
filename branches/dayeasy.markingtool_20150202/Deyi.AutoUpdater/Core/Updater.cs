...
namespace Deyi.AutoUpdater.Core
{
    public class Updater
    {
            ...
            ThreadPool.QueueUserWorkItem(s =>
            {
                ...
                client.DownloadDataCompleted += (x, y) =>
                {
                    try
                    {
                        ...
                        {
                            var ser = new DataContractJsonSerializer(typeof (ApiResult<ManifestInfo>));
                            var result = (ApiResult<ManifestInfo>) ser.ReadObject(stream);
        ...
        public void StartUpdate(ManifestInfo updateInfo)
        {
            if (Instance.CurrentVersion >= updateInfo.AppVersion)
            {
                //当前版本是最新的，不更新
                return;
            }
