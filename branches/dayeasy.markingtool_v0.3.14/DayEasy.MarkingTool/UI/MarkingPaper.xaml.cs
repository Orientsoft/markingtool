...
namespace DayEasy.MarkingTool.UI
{
    ...
    public partial class MarkingPaper
    {
        ...
        {
            _usage = usage;
            _fileManager = new FileManager(usage.Batch, usage.PrintType);
        ...
        private void MarkingPaper_Closing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            {
                _fileManager.SaveMarkedResult(_markedInfoList, _markedResult);
            }
        }
        ...
        private void BeginPorgress(object args)
        {
            ...
            if (_batchDetail == null)
                return;
            int i = 0;
            int taskCount = DeyiKeys.Section.SizeConfig.TaskCount;
            List<Task> tasks = null;
            if (taskCount > 1)
                tasks = new List<Task>(taskCount);
            foreach (var item in _paperPathsList)
            {
                ...
                {
                    SingleProcess(imagePath, manager);
                    TaskFinished(i);
                }
                else
                {
                    ...
                    {
                        var task = Task.Factory.StartNew(() => SingleProcess(imagePath, manager))
                            .ContinueWith(t => TaskFinished(i));
                        tasks.Add(task);
                    }
                    else
                    {
                        Task.Factory.ContinueWhenAny(tasks.ToArray(), t => SingleProcess(imagePath, manager))
                            .ContinueWith(t => TaskFinished(i));
                    }
                }
            }
        ...
        private void TaskFinished(int index)
        {
            Dispatcher.Invoke(new Action(() =>
            {
                int count = _paperPathsList.Count;
                LblForBar.Content = (index + 1) + "/" + count;
        ...
        private void SingleProcess(string imagePath, ScannerBuilderManager manager)
        {
            var marked = new PaperMarkedInfo
            {
                ImagePath = imagePath
            };
            var builder = new PaperScannerBuidler(imagePath, _batchDetail, _usage.ClassId, _fileManager);
            try
            {
        ...
        private void btnOperate_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (markedInfo.IsSuccess)
            {
                ...
                var marking = new MarkingSubjective(markedInfo, _batchDetail, ref _markedResult, _kind, _fileManager);
        ...
        {
            _markedInfoList = _fileManager.LoadMarkedResult(out _markedResult);
            if (!_markedInfoList.Any())
                return;
        ...
        private void UploadMarkedPaper(object arg)
        {
            try
            {
                ...
                foreach (var item in markedList)
                {
                    ...
                    if (!item.IsUpload)
                    {
                        WindowsHelper.SetMarkedSymbols(resultItem, _fileManager.SliceDirectory, item.PaperName);
                        var zipPath = _fileManager.PacketMarkedPicture(item.PaperName);
                        var data = new PaperMarkingFileData
                        {
                            Data = File.OpenRead(zipPath)
                        };
        ...
        {
            var onlyFail = OnlyFail.IsChecked ?? true;
            var msg = onlyFail
                ? "是否确认删除所有识别失败的阅卷结果？"
                : "是否确认删除所有阅卷结果？";
            if (!WindowsHelper.ShowQuestion(msg))
        ...
        private void btnPrintAll_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                var size = ScannerType.SelectedIndex++;//(((ComboBoxItem)ScannerType.SelectedItem).Content.ToString() == "单面" ? 1 : 2);
        ...
        private void btnDelete_Click(object sender, RoutedEventArgs e)
        {
            ...
            DeleteMarking(m => m.MarkedResultId == id);
            ShowMsg();
        }
        ...
        private void EnabledButton(bool isDisable)
        {
            Dispatcher.Invoke(new Action(() =>
            {
                ...
                ComboSort.IsEnabled = isDisable;
                OnlyFail.IsEnabled = isDisable;
                ScannerType.IsEnabled = isDisable;
            }));
        }
        ...
        {
            if (!IsLoaded) return;
            var sortType = (ComboSort.SelectedIndex == 0 ? SortType.Default : SortType.IndexDesc);
            MarkingSort(sortType);
        }
        ...
        private void btnSee_Click(object sender, RoutedEventArgs e)
        {
            ...
            var win = new PaperImageDetail(imgPath);
            win.ShowDeyiDialog();
        }
        ...
        private void MarkingSort(SortType sortType)
        {
            EnabledButton(false);
            Func<PaperMarkedInfo, decimal> orderBy;
            bool isDesc = sortType.IsDesc();
            switch (sortType)
            {
                case SortType.Score:
                ...
                case SortType.ErrorCountDesc:
                    orderBy = t => t.ErorrCount;
                    break;
                default:
                    orderBy = t => t.Index;
                    break;
            ...
            tempList = (isDesc
                ? tempList.OrderByDescending(orderBy).ToList()
                : tempList.OrderBy(orderBy).ToList());
            _markedInfoList.Clear();
