    ...
    public class AnswerSheetHelper : IDisposable
    {
        ...
        private void CutAnswerArea()
        {
            ...
            if (DeyiKeys.WriteFile)
                _sheetBitmap.Save(Path.Combine(DeyiKeys.MarkingPath, DeyiKeys.DebugName,
                    string.Format("answerSheet_{0}.png", Helper.Guid32.Substring(5, 6))));
        }
        ...
        private PointF GetLeftTopPointF()
        {
            for (var x = 0; x < _width / 4; x++)
            {
                ...
                {
                    var isBlack = ScanerBlackCount(x, y, Threshold);
                    if (isBlack)
                    {
        ...
        /// <returns></returns>
        private bool ScanerBlackCount(int x, int y, int threshold)
        {
            ...
            {
                if (GetPixel(x + w*i, y).IsBlack())
                    xBlack++;
                if (GetPixel(x, y + w*i).IsBlack())
                    yBlack++;
            }
            return xBlack >= (threshold*0.9F) || (xBlack + yBlack) >= threshold*Scale;
        }
