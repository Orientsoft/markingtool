...
namespace DayEasy.MarkingTool.BLL
{
    ...
    public class HttpHelper : IDisposable
    {
        ...
        private string _userAgent = Helper.UserAgent();
        ...
        public HttpHelper(string url)
            : this(url, Empty, Encoding.Default, Empty, Empty, Empty)
        {
        }
        ...
        public HttpHelper(string url, Encoding encoding)
            : this(url, Empty, encoding, Empty, Empty, Empty)
        {
        }
        ...
        private void CreateHttpRequest()
        {
            ...
            if (!string.IsNullOrEmpty(_referer))
            {
                if (_referer == "base")
                {
                    var baseUrl = new Regex("http(s)?://([^/]+?)/", RegexOptions.IgnoreCase).Match(_url).Groups[2].Value;
        ...
        private string GetContentType(string fileName)
        {
            ...
            if (string.IsNullOrWhiteSpace(ext) || !DeyiKeys.ContentTypes.ContainsKey(ext))
                return DeyiKeys.ContentTypes["*"];
            return DeyiKeys.ContentTypes[ext];
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Stream GetStream(out string msg)
        {
            msg = Empty;
            CreateHttpRequest();
            if (_req == null)
        ...
        public bool SaveFile(string path)
        {
            ...
            string msg;
            var resStream = GetStream(out msg);
            if (resStream == null)
                return false;
            try
            {
