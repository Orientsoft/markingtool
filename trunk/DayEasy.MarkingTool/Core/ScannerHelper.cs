...
namespace DayEasy.MarkingTool.Core
{
    ...
    public class ScannerHelper : IDisposable
    {
        ...
        private List<string> _scannerList;

        public ScannerHelper(Grid parentGrid, string prefix, bool isSingle = true)
        {
            _isSingle = isSingle;
            _prefix = prefix;
            ...
            _host = new WindowsFormsHost { Child = _amx, Width = 0, Height = 0 };
            parentGrid.Children.Add(_host);
            Grid.SetRow(_host, 0);
            _scannerList = new List<string>();

        }

        public List<string> Scnaner(int startIndex = 1)
        {
            _scannerList = new List<string>();
            _amx.BeginInit();
        ...
        public static bool CheckOcx()
        {
            ...
            var rkTest = Registry.ClassesRoot.OpenSubKey("TypeLib\\" + id + "\\");
            return rkTest != null;
        }
        ...
        public void Dispose()
        {
            if (_amx != null)
                _amx.Dispose();
            if (_host != null && _parentGrid != null)
            {
                _parentGrid.Children.Remove(_host);
            }
        }
    }
}
